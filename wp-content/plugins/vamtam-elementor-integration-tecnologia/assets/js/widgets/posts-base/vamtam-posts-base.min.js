var defaultPosts = elementorModules.frontend.handlers.Base.extend({
    ...
    fitImages() {
        var e = jQuery,
            ...
            n = this.getSettings();
        this.elements.$postsContainer.toggleClass(n.classes.hasItemRatio, !!s.match(/\d/)), !t.isMasonryEnabled() && (this.elements.$posts = this.elements.$postsContainer.find(n.selectors.post), this.elements.$posts.each(function() {
            var s = e(this),
...
class VamtamLoadMore extends elementorModules.frontend.handlers.Base {
    ...
    onInit() {
        ...
        let e = this.getElementSettings("pagination_type");
    ...
    handleSuccessFetch(e) {
        ...
        let t = e.querySelectorAll(`[data-id="${this.elementId}"] .elementor-posts-container > article`),
            s = e.querySelector(".e-load-more-anchor").getAttribute("data-next-page"),
            n = [...t].reduce((e, t) => e + t.outerHTML, "");
        this.elements.postsContainer.insertAdjacentHTML("beforeend", n), this.elements.loadMoreAnchor.setAttribute("data-page", this.currentPage), this.elements.loadMoreAnchor.setAttribute("data-next-page", s), this.currentPage === this.maxPage && this.handleUiWhenNoPosts(), setTimeout(() => {
            jQuery(window).trigger("resize")
        }, 10)
    }
    handlePostsQuery() {
        this.handleUiBeforeLoading(), this.currentPage++;
        let e = new URL(this.elements.loadMoreAnchor.getAttribute("data-next-page"));
        return e.searchParams.set("vamtam_posts_fetch", 1), fetch(e.toString()).then(e => e.text()).then(e => {
            let t = new DOMParser,
                s = t.parseFromString(e, "text/html");
            this.handleSuccessFetch(s)
        }).catch(e => {
...
class VamtamMasonry extends elementorModules.frontend.handlers.Base {
    onInit() {
        elementorModules.frontend.handlers.Base.prototype.onInit.apply(this, arguments), this.bindEvents(), this.loadMoreMasonryFix(), this.checkApplySafariFix()
    }
    checkApplySafariFix() {
        let e = this;
        jQuery(window).on("load", () => {
            jQuery("html").hasClass("safari") && setTimeout(() => {
                e.onWindowResize()
            }, 10)
        })
    }
    ...
    checkDiscardDuplicates() {
        let e = this.$element,
            t = e.find(".elementor-posts-container"),
            s = t.find(".elementor-post:visible"),
            n = jQuery(".vamtam-blog-featured-post .elementor-post:visible"),
            i = [],
            o = [],
            a = 0;
        if (s.length) {
            if (jQuery.each(s, (e, t) => {
                    let n = parseInt(s[e].classList[2].match(/\d+/)[0]);
                    n && !isNaN(n) && (-1 === i.indexOf(n) ? i.push(n) : (jQuery(t).remove(), a++))
                }), n.length) {
                jQuery.each(n, e => {
                    let t = parseInt(n[e].classList[2].match(/\d+/)[0]);
                    t && !isNaN(t) && -1 === o.indexOf(t) && o.push(t)
                });
                let r = t.find(".elementor-post:visible");
                jQuery.each(r, (e, t) => {
                    let s = parseInt(r[e].classList[2].match(/\d+/)[0]);
                    s && !isNaN(s) && -1 !== o.indexOf(s) && (jQuery(t).remove(), a++)
                })
            }
            return a
        }
    ...
    loadMoreMasonryFix() {
        let e = this.getElementSettings().pagination_type;
        if ("load_more_on_click" !== e && "load_more_infinite_scroll" !== e) return;
        var t = this.$element,
            s = t.find(".elementor-posts-container"),
            n = t.find(".elementor-button"),
            i = t.find(".e-load-more-anchor"),
            o = null,
            a = null,
            r = null,
            l = this;
        let h = () => {
                "load_more_on_click" === e ? jQuery(n).off("click", d) : r.disconnect()
            },
            d = () => {
                o = s.find(".elementor-post:visible").length;
                let e = !1,
                    n = setInterval(() => {
                        if (o !== (a = s.find(".elementor-post:visible").length)) {
                            let d = l.checkDiscardDuplicates();
                            l.recalculateMasonry(), o = a - d, clearInterval(n), e = !0, r && r.observe(i[0])
                        }
                        t.hasClass("e-load-more-pagination-end") && h()
                    }, 50);
                e || setTimeout(() => {
                    clearInterval(n)
                }, 1e4)
            };
        "load_more_on_click" === e ? jQuery(n).on("click", d) : (() => {
            !this.isEdit && (r = elementorModules.utils.Scroll.scrollObserver({
                callback(e) {
                    e.isInViewport && (r.unobserve(i[0]), d())
                }
            })).observe(i[0])
        })()
    }
    getSkinPrefix() {
        if (this.skinPrefix) return this.skinPrefix;
        let e = this.getSettings() ? .elementName ? .split(".")[1];
        return e ? (this.skinPrefix = e + "_", this.skinPrefix) : "vamtam_classic_"
    }
    bindEvents() {
        var e = this.getModelCID();
        elementorFrontend.addListenerOnce(e, "resize", this.onWindowResize.bind(this))
    }
    getClosureMethodsNames() {
        return elementorModules.frontend.handlers.Base.prototype.getClosureMethodsNames.apply(this, arguments).concat(["fitImages", "onWindowResize", "runMasonry"])
    }
...
jQuery(window).on("elementor/frontend/init", () => {
    if (elementorFrontend.elementsHandler && elementorFrontend.elementsHandler.attachHandler) VAMTAM_FRONT.elementor.widgets.isWidgetModActive("posts") && (elementorFrontend.elementsHandler.attachHandler("posts", defaultPosts, "vamtam_classic"), elementorFrontend.elementsHandler.attachHandler("posts", VamtamLoadMore, "vamtam_classic"), elementorFrontend.elementsHandler.attachHandler("posts", VamtamMasonry, "vamtam_classic"), elementorFrontend.elementsHandler.attachHandler("posts", VamtamMasonry, "classic")), VAMTAM_FRONT.elementor.widgets.isWidgetModActive("archive-posts") && (elementorFrontend.elementsHandler.attachHandler("archive-posts", defaultPosts, "vamtam_classic"), elementorFrontend.elementsHandler.attachHandler("archive-posts", VamtamLoadMore, "vamtam_classic"), elementorFrontend.elementsHandler.attachHandler("archive-posts", VamtamMasonry, "vamtam_classic"), elementorFrontend.elementsHandler.attachHandler("archive-posts", VamtamMasonry, "archive_classic"));
    else {
        let e = e => {
